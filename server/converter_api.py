...
def pdf_to_xlsx(
    ...
    """Fast PDF -> XLSX extractor.

    Uses pdfplumber table extraction first (native PDF tables), then a lightweight
    PyMuPDF word/column reconstruction fallback. OCR is used only for pages with
    insufficient native text, avoiding the expensive OCR path for normal PDFs.
    """
    ...
    try:
        ...
        if pdf.page_count > MAX_PDF_PAGES:
            raise HTTPException(status_code=413, detail=f"PDF contains more than {MAX_PDF_PAGES} pages.")

        # Optional table engine. It is deliberately attempted once per page and
        # only falls back to coordinate grouping when it cannot find a table.
        try:
            import pdfplumber
        ...
        try:
            for page_index, page in enumerate(pdf):
                ws = workbook.create_sheet(f"Page {page_index + 1}")
                ws.sheet_view.showGridLines = False
                wrote = False

                if plumber is not None:
                    try:
                        ppage = plumber.pages[page_index]
                ...
                # Lightweight native text fallback. This is much cheaper than OCR.
                text = page.get_text("text") or ""
                if len(text.strip()) >= OCR_MIN_TEXT_CHARS and not wrote:
                    words = page.get_text("words", sort=True) or []
                    lines = group_words_into_lines(words)
                    for row_no, line in enumerate(lines, 1):
                        # Split a line into approximate columns using large x gaps.
                        word_items = line.get("words") or []
                        values = []
                        current = []
                        prev_x1 = None
                        for word in word_items:
                            x0 = float(word.get("x0", 0))
                            if prev_x1 is not None and x0 - prev_x1 > max(24, float(line.get("height", 10)) * 2.2):
                                values.append(" ".join(current))
                                current = []
                            current.append(str(word.get("text") or "").strip())
                            prev_x1 = float(word.get("x1", x0))
                        if current:
                            values.append(" ".join(current))
                        if not values:
                            values = [str(line.get("text") or "")]
                        for col_no, value in enumerate(values, 1):
                            ws.cell(row_no, col_no, value)
                    wrote = bool(lines)
                    if wrote:
                ...
                # OCR only when the page genuinely has no usable native text.
                if not wrote:
                    ...
                    for row_no, line in enumerate(lines, 1):
                        ...
                        if text_value:
                            ws.cell(row_no, 1, text_value)
                    wrote = bool(lines)

                if not wrote:
                    ws.cell(1, 1, "No readable content found on this page.")

                # Professional spreadsheet defaults.
                for column_cells in ws.columns:
                    max_len = 0
                    col_letter = column_cells[0].column_letter
                    for cell in column_cells:
                        cell.alignment = Alignment(vertical="top", wrap_text=True)
                        max_len = min(55, max(max_len, len(str(cell.value or ""))))
        ...
        raise HTTPException(status_code=500, detail=f"Could not create Excel workbook: {exc}")
    finally:
        pdf.close()
        try:
            workbook.close()
    ...
    return verify_output(output, "PDF → Excel")
